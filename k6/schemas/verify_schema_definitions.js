...
export function checkAndBuildCRDArray(cookies, crdArray) {
  ...
  while (crdArray.length != crdCount && attempts < retries) {
    ...
    for (let i = 0; i < crdCount; i++) {
      ...
      k6Util.trackDataMetricsPerURL(res, crdUtil.crdsTag, headerDataRecv, epDataRecv)
      sleep(0.25)
    }
...
export function generateCRDArray(cookies) {
  ...
  k6Util.trackDataMetricsPerURL(res, crdUtil.schemaDefinitionTag, headerDataRecv, epDataRecv)
...
export function verifySchemas(data) {
  ...
  CRDs.forEach((crd, i) => {
    ...
    k6Util.trackDataMetricsPerURL(res, crdUtil.crdTag, headerDataRecv, epDataRecv)
    ...
    k6Util.trackDataMetricsPerURL(res, crdUtil.putCRDTag, headerDataRecv, epDataRecv)
    updated += 1
  })
  ...
  existingIDs.forEach(id => {
    ...
    k6Util.trackDataMetricsPerURL(res, crdUtil.schemasTag, headerDataRecv, epDataRecv)
