...
func InitModal(c *cli.Context, d *os.File) modalUiModel {

	rand.New(rand.NewSource(time.Now().UTC().UnixNano()))
	modals := make(map[string]tea.Model)
	modals["root"] = NewRootTUIModel(c, d)
	modals["dart"] = NewDartPickerModel()
...
func Start(c *cli.Context) error {
	...
	m := InitModal(c, dump)
	model := m.modals[m.selectedModal]
